...
spark.sql(f"""CREATE OR REPLACE POLICY nation_filter
...
spark.sql(f"""CREATE OR REPLACE POLICY mask_address
              ON SCHEMA {catalog_name}.{schema_name}
              ...
              FOR TABLES
              MATCH COLUMNS
                hasTagValue('pii', 'address') AS address
              ON COLUMN address
""")
...
spark.sql(f"ALTER TABLE {catalog_name}.{schema_name}.customers ALTER COLUMN c_address SET TAGS ('pii' = 'address')")

spark.sql(f"ALTER TABLE {catalog_name}.{schema_name}.suppliers SET TAGS ('geo')")
spark.sql(f"ALTER TABLE {catalog_name}.{schema_name}.suppliers ALTER COLUMN n_name SET TAGS ('geo')")
